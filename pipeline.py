...
if uploaded_file is not None:
    ...
    feedbacks_df = copy(initial_feedbacks_df)
    ...
    if st.button("Run"):
        ...
        prompt_feedback = PromptTemplate.from_template(
            ...
            partial_variables= {"format_instructions": parser_feedback.get_format_instructions()},
        )
        prompt_and_model_feedback = prompt_feedback | model | parser_feedback

        #feedback_categoriser = create_feedback_categoriser(feedback_context)
        
        first_insights_list = []

        for i in stqdm(range(2)): #REMOVE 2
            feedback = feedbacks_df[feedbacks_column].iloc[:3]
